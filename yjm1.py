...
def dataSampling(**kwargs):
    if kwargs['datatype'] == 'int':
        ...
        if kwargs['datatype'] == 'str':
            if isinstance(kwargs['datarange'], tuple) and len(kwargs['datarange']) == 2:#包含两个元素的元组，代表字符串长度范围
                return ''.join(random.choices(string.ascii_letters, k=kwargs['datarange'][1]))
            else:
                return ''.join(random.choices(string.ascii_letters, k=kwargs['datarange']))#生成指定长度的字符串。
