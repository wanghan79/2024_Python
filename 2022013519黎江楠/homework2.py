    ...
    def data_sampling(**kwargs):
        ...
        if typ == int:
            ...
            for k, v in inner.items():
                result.append(DataSamplingOperation.data_sampling(**v))
            return tuple(result)
        elif typ == set:
            result = set()
            for k, v in inner.items():
                result.add(DataSamplingOperation.data_sampling(**v))
            return result
        elif typ == list:
            result = list()
            for k, v in inner.items():
            ...
            result = dict()
            for k, v in inner.items():
                result[k] = DataSamplingOperation.data_sampling(**v)
            return typ(**result)

    @staticmethod
    def calculate(struct, operation):
        qu = queue.Queue(0)
        qu.put(struct)
        operation.init_func()
        while not qu.empty():
            frt = qu.get()
            operation.midway_func(frt)
...
if __name__ == '__main__':
    ...
    print(obj)
    print(DataSamplingOperation.calculate(obj, GetAverage(int, float)))
    print(DataSamplingOperation.calculate(obj, GetSum(int)))
