...
class RandomDataStructure:
    ...
    def generate_structure(self, **kwargs):
        ...
        if datatype == 'int':
            ...
            return [self.generate_structure(**sub) for sub in subs.values()]
        elif datatype == 'tuple':
            return tuple(self.generate_structure(**sub) for sub in subs.values())
        else:
            raise ValueError(f"不支持的数据类型: {datatype}")

    def collect_numbers(self, data, int_sum=0, int_count=0, float_sum=0, float_count=0):
        if isinstance(data, int):
            ...
            for item in data:
                int_sum, int_count, float_sum, float_count = self.collect_numbers(item, int_sum, int_count, float_sum,
                                                                                  float_count)
        return int_sum, int_count, float_sum, float_count

    def datastructure_calculator(self, calc_type):
        def decorator(func):
            @wraps(func)
            def wrapper(n, **kwargs):
                ...
                random_datas = func(n, **kwargs)

                for data in random_datas:
                    int_sum, int_count, float_sum, float_count = self.collect_numbers(data)
    ...
    def main(self, n, calc_type, **kwargs):
        @self.datastructure_calculator(calc_type)
        def generate_random_datas(n, **kwargs):
            return [self.generate_structure(**kwargs) for _ in range(n)]

        return generate_random_datas(n, **kwargs)
...
if __name__ == "__main__":
    ...
    random_data_structure = RandomDataStructure()
    random_datas = random_data_structure.main(n, calc_type, **data_structure)
