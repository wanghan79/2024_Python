...
class func(object):
    ...
    def tocaldata(self,datatype):
        ...
        elif(datatype == "float"): flag = 0
        elif (datatype == "float_tuple"): flag = 3
        elif (datatype == "float_list"): flag = 4
        elif (datatype == "int_tuple"): flag = 5
        elif (datatype == "int_list"): flag = 6
        for key in self.res:
            if flag == 1:
                if type(key) == float or type(key) == str: continue
                elif type(key) == int:
...
def test(operator):
    def decorator(functional):
        @wraps(functional)
        def wrapper(*args,**kwargs):
            fobject=func(**dic)
            fobject.datasampling()
            fobject.exportdata(operator)
            fobject.cnt = 0
            fobject.sum = 0
            return functional()
        return wrapper
...
#基本数据结构
struct = {
    "int": {"datatype": int, "range": (0, 999)},
    "float": {"datatype": float, "range": (0, 99)},
    "str": {"datatype": str, "range": string.ascii_lowercase, "len": 5},
